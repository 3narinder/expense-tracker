...
app.use(
  cors({
    origin: (origin, callback) => {
      ...
      // Exact allowlist (env-configured or fallback list)
      if (allowedOrigins.includes(origin)) {
        return callback(null, true);
      }

      // In development, accept localhost/127.0.0.1 variations on any port
      if (process.env.NODE_ENV !== "production") {
        try {
          ...
          // if origin isn't a valid URL, fall through to rejection
        }
      }

      ...
      <nav className="flex-1 p-3 space-y-1.5">
        {navItems?.map(({ to, label, icon: Icon }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              ` relative flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-medium transition ${isActive ? "bg-slate-100 text-slate-900 before:absolute before:left-0 before:top-2.5  before:bottom-2.5 before:w-1 before:rounded-full before:bg-violet-500" : "text-slate-700 hover:bg-slate-50"}`
            }
          >

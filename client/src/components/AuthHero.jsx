...
const BalanceCard = () => (
  <div className="bg-(--color-accent) rounded-2xl p-5 text-(--color-accent-foreground) shadow-xl relative overflow-hidden">
    <div className="absolute -top-8 -right-8 h-24 w-24 bg-(--color-accent-foreground)/10 rounded-full blur-2xl" />
...
const AIInsightCard = () => (
  <div className="bg-(--color-bg-surface) rounded-2xl p-4 shadow-lg border border-(--color-border-main)">
    <div className="flex items-start gap-2.5">
      <div className="h-9 w-9 rounded-xl bg-(--color-accent) flex items-center justify-center shrink-0">
        <Sparkles size={16} className="text-(--color-accent-foreground)" />
      </div>
      <div className="min-w-0">
        <div className="text-[10px] uppercase tracking-wider font-bold text-(--color-text-muted) mb-0.5">
          AI Insight
        </div>
...
const BudgetProgressCard = () => (
  <div className="bg-(--color-bg-surface) rounded-2xl p-4 shadow-md border border-(--color-border-main)">
    <div className="flex items-center gap-2 mb-3">
      <div className="h-8 w-8 rounded-lg bg-(--color-bg-muted) flex items-center justify-center">
        <Utensils size={14} className="text-(--color-warning)" />
      </div>
      <div>
    ...
    <div className="h-1.5 bg-(--color-bg-muted) rounded-full overflow-hidden">
      <div
        className="h-full bg-(--color-warning) rounded-full"
        style={{ width: "80%" }}
      />
    </div>
    <div className="text-[10px] text-(--color-warning) font-medium mt-1.5">
      80% used
    </div>
  </div>
);
      ...
      {[
        ...
        <div key={s.name} className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="h-7 w-7 rounded-lg bg-(--color-accent) flex items-center justify-center text-(--color-accent-foreground) text-[11px] font-bold">
              {s.initial}
            </div>
          ...
          <span className="text-[11px] font-semibold text-(--color-text-main)">
            {s.cost}
          </span>
        </div>
      ))}
  ...
  <div className="bg-(--color-bg-surface) rounded-2xl p-4 shadow-md border border-(--color-border-main)">
    ...
    <div className="grid grid-cols-2 gap-3 mb-3">
      <div>
        ...
        <div className="text-base font-bold text-(--color-success)">$6.3k</div>
      </div>
      <div>
        <div className="text-[10px] text-(--color-text-muted)">Expense</div>
        <div className="text-base font-bold text-(--color-danger)">$2.4k</div>
      </div>
    </div>
    <div className="flex items-end gap-1 h-10">
      {[55, 70, 45, 80, 65, 75].map((h, i) => (
        <div key={i} className="flex-1 flex flex-col gap-0.5 justify-end">
          <div
            className="bg-(--color-success) rounded-sm opacity-70"
            style={{ height: `${h}%`, minHeight: "4px" }}
          />
          <div
            className="bg-(--color-danger) rounded-sm opacity-70"
...
const MonthlySummaryCard = () => (
  <div className="bg-(--color-bg-surface) rounded-2xl p-4 shadow-md border border-(--color-border-main)">
    <div className="flex items-center justify-between mb-3">
      ...
      <span className="text-[10px] text-(--color-success) bg-(--color-bg-muted) px-1.5 py-0.5 rounded-full font-bold">
        Healthy
      </span>
    </div>
      ...
      {[
        ...
        <div key={i} className="flex items-center justify-between">
          <div className="flex items-center gap-2 min-w-0">
            <div className="h-7 w-7 rounded-lg bg-(--color-bg-muted) flex items-center justify-center shrink-0 text-(--color-text-muted)">
              <t.icon size={12} />
            </div>
  ...
  <div className="bg-(--color-bg-surface) rounded-2xl p-4 shadow-md border border-(--color-border-main)">
    ...
    <div className="flex items-center gap-3">
      ...
      <div className="space-y-1.5 flex-1 min-w-0">
        {[
          { name: "Food", value: "$320", color: "bg-(--color-accent)" },
          { name: "Rent", value: "$1.8k", color: "bg-(--color-warning)" },
          { name: "Travel", value: "$240", color: "bg-(--color-info)" },
        ].map((c) => (
          <div
            key={c.name}
...
const SavingsGoalCard = () => (
  <div className="bg-(--color-bg-surface) rounded-2xl p-4 shadow-md border border-(--color-border-main)">
    <div className="flex items-center gap-2 mb-3">
      <div className="h-8 w-8 rounded-lg bg-(--color-accent) flex items-center justify-center">
        <Plane size={14} className="text-(--color-accent-foreground)" />
      </div>
      <div>
        <div className="text-xs font-semibold text-(--color-text-main)">
          Vacation Fund
    ...
    <div className="flex items-baseline justify-between mb-1.5">
      ...
      <span className="text-[10px] text-(--color-success) font-bold">65%</span>
    </div>
    <div className="h-1.5 bg-(--color-bg-muted) rounded-full overflow-hidden">
      <div
...
const UpcomingBillCard = () => (
  <div className="bg-(--color-bg-surface) rounded-2xl p-4 shadow-md border border-(--color-border-main)">
    <div className="flex items-center gap-3">
      <div className="h-10 w-10 rounded-xl bg-(--color-bg-muted) flex items-center justify-center shrink-0">
        <Calendar size={16} className="text-(--color-danger)" />
      </div>
      <div className="flex-1 min-w-0">
        <div className="text-[10px] uppercase tracking-wider text-(--color-danger) font-bold">
          Due in 2 days
        </div>
        <div className="text-xs font-semibold text-(--color-text-main)">
          Rent payment
...
const PortfolioCard = () => (
  <div className="bg-(--color-accent) rounded-2xl p-4 text-(--color-accent-foreground) shadow-xl">
    <div className="flex items-center justify-between mb-3">
...
const AuthHero = ({ headline, subHeadline }) => {
  return (
    <div
      data-theme="light"
      className="relative h-full w-full overflow-hidden bg-(--color-bg-app)"
    >
      <div className="absolute top-1/2 -right-24 w-80 h-80 bg-(--color-accent)/10 rounded-full blur-3xl pointer-events-none z-0" />
      ...
      <div className="absolute top-0 left-0 right-0 h-64 bg-linear-to-b from-(--color-bg-app) via-(--color-bg-app)/90 to-transparent pointer-events-none z-20" />
      <div className="absolute bottom-0 left-0 right-0 h-40 bg-linear-to-t from-(--color-bg-app) via-(--color-bg-app)/70 to-transparent pointer-events-none z-20" />
